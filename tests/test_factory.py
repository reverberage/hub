...
class TestListProviders:
    def test_returns_list(self) -> None:
        ...
        # fallback should NOT be in the list
...
class TestGetProvider:
    ...
    def test_env_var_default(self) -> None:
        """RVRB_PROVIDER env is read when name is None."""
        with patch("openai.OpenAI"), patch.dict(
            "os.environ", {"RVRB_PROVIDER": "openai", "OPENAI_API_KEY": "k"}
        ):
            p = get_provider()
            assert p.model == "gpt-4"

    def test_explicit_name(self) -> None:
        """Explicit name overrides env."""
        with patch("openai.OpenAI"), patch.dict(
            "os.environ", {"RVRB_PROVIDER": "openai", "DASHSCOPE_API_KEY": "k"}
        ):
            p = get_provider("qwen")
            assert isinstance(p, ModelProvider)
            assert p.model == "qwen3-coder-plus"
    ...
    def test_model_override_colon(self) -> None:
        """qwen:model-id passes model override."""
        with patch("openai.OpenAI"), patch.dict("os.environ", {"DASHSCOPE_API_KEY": "k"}):
...
class TestGetProviderFallback:
    def test_fallback_from_env(self) -> None:
        with patch("openai.OpenAI"), patch.dict(
            "os.environ",
            {
                "RVRB_FALLBACK_PROVIDERS": "qwen,local",
                "DASHSCOPE_API_KEY": "k",
            },
        ):
    ...
    def test_fallback_env_missing(self) -> None:
        with patch.dict("os.environ", clear=True):
            with pytest.raises(ValueError, match="RVRB_FALLBACK_PROVIDERS"):
    ...
    def test_fallback_env_mixed_model_overrides(self) -> None:
        with patch("openai.OpenAI"), patch.dict(
            "os.environ",
            {
                "RVRB_FALLBACK_PROVIDERS": "qwen:model-a,qwen:model-b",
                "DASHSCOPE_API_KEY": "k",
            },
        ):
            p = get_provider("fallback")
            assert isinstance(p, FallbackProvider)
            # Access internal providers to verify model overrides
