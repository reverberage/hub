...
from rvrb_providers.models import ProviderError, QuotaExhaustedError
from rvrb_providers.qwen import QwenProvider, _is_free_tier_exhausted
...
class TestComplete:
    ...
    def test_sends_correct_payload(
        self, provider: QwenProvider, mock_create: MagicMock
    ) -> None:
    ...
    def test_429_free_tier_exhausted(
        self, provider: QwenProvider, mock_create: MagicMock
    ) -> None:
    ...
    def test_429_without_free_tier(
        self, provider: QwenProvider, mock_create: MagicMock
    ) -> None:
        mock_create.side_effect = make_429_error(
...
class TestCompleteStructured:
    ...
    def test_sends_response_format(
        self, provider: QwenProvider, mock_create: MagicMock
    ) -> None:
        ...
        provider.complete_structured(
            [{"role": "user", "content": "Extract"}], Item
        )
        call_kwargs = mock_create.call_args[1]
...
class TestCompleteWithTools:
    ...
    def test_sends_tool_choice_auto(
        self, provider: QwenProvider, mock_create: MagicMock
    ) -> None:
        mock_msg = MagicMock()
        mock_msg.content = "ok"
